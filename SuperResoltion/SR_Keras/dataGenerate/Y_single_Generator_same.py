...
def image_patch(_datapath, _patchsize, _interval):
    ...
    for file in files:
        ...
        image = cv2.cvtColor(image, cv2.COLOR_BGR2YCrCb)  # 转化到YCrCb空间
        input = cv2.cvtColor(input, cv2.COLOR_BGR2YCrCb)

        image_Y = image[:, :, 0]  # 提取Y分量
        input_Y = input[:, :, 0]
        label = image_Y.astype('float64') / 255.
        input_Y = input_Y.astype('float64') / 255.
        ...
        for i in range(coor.shape[1]):
            ...
            labelPatch = labelPatch[:, :, np.newaxis]  # 增加通道数的维度
        ...
        for i in range(coor.shape[1]):
            x_patch, y_patch = coor[0][i], coor[1][i]
            input_Y_patch = input_Y[y_patch - _patchsize // 2:y_patch + _patchsize // 2,
                            x_patch - _patchsize // 2:x_patch + _patchsize // 2]  # 取patch

            input_Y_patch = input_Y_patch[:, :, np.newaxis]  # 增加通道数的维度
            # print(image_x2_patch.shape)
            InputImgs.append(input_Y_patch)

    print(InputImgs.__len__())  # 两倍下采样图像序列的大小
...
if __name__ == "__main__":
    ...
    scale = 4
    label_patchsize = 72
    train_H5 = "../data/H5/Yang91/{}_91Train_x{}.h5".format("Y", scale)
    test_H5 = "../data/H5/Yang91/{}_91Test_x{}.h5".format('Y', scale)
